...
export function Navbar({ cart = [], onRemoveFromCart }: NavbarProps) {
  ...
  useEffect(() => {
    ...
    if (previous > 0 && current > previous) {
      ...
      toast.info(`${added} new notification${added > 1 ? "s" : ""} received`);
    }
        ...
        >
          ...
          >
            ...
            {serviceMenuItems.map((item) => (
              <Button
                key={item.path}
                variant={isNavItemActive({ path: item.path }) ? "default" : "ghost"}
                size="sm"
                className={isNavItemActive({ path: item.path }) ? "" : "text-muted-foreground"}
                onClick={() => handleMenuNavigation(item.path)}
              >
                {item.label}
              </Button>
            ...
            {/* Individual dashboard menu items */}
            {isAuthenticated && dashboardPaths.map((dashboard) => (
              <Button
                key={dashboard.role}
                variant={isNavItemActive({ path: dashboard.path }) ? "default" : "ghost"}
                size="sm"
                className={isNavItemActive({ path: dashboard.path }) ? "" : "text-muted-foreground"}
          ...
          <div className="flex shrink-0 items-center gap-2 sm:gap-3">
            <Button
              ...
              aria-label={
                isNavbarMinimized ? "Expand navigation" : "Minimize navigation"
              }
              title={
                isNavbarMinimized ? "Expand navigation" : "Minimize navigation"
              }
                                ...
                                <>
                                  ...
                                  {Number(n.recipientCount || 0) > 0 ? (
                                    <span className="text-xs text-muted-foreground block mt-1">
                                      Audience: {Number(n.recipientCount)}{" "}
                                      user(s)
                                    </span>
                                  ) : null}
                                </>
          ...
          <div className="flex gap-2 overflow-x-auto whitespace-nowrap rounded-full border border-border/70 bg-background/55 p-1">
            ...
            {serviceMenuItems.map((item) => (
              <Button
                ...
                variant={isNavItemActive({ path: item.path }) ? "default" : "ghost"}
                size="sm"
                className={`rounded-full px-4 ${isNavItemActive({ path: item.path }) ? "" : "text-muted-foreground"}`}
                onClick={() => handleMenuNavigation(item.path)}
              >
                {item.label}
              </Button>
            ...
            {isAuthenticated && dashboardPaths.map((dashboard) => (
              <Button
                ...
                variant={isNavItemActive({ path: dashboard.path }) ? "default" : "ghost"}
                size="sm"
                className={`rounded-full px-4 ${isNavItemActive({ path: dashboard.path }) ? "" : "text-muted-foreground"}`}
                onClick={() => handleMenuNavigation(dashboard.path)}
              >
                {dashboard.label}
              </Button>
